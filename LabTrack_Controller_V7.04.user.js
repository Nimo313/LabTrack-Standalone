//LabTrack_Controller_V7.04_Enhanced.user.js
// ==UserScript==
// @name         LabTrack Controller (V7.04 ENHANCED - Performance & Modern ES6+)
// @namespace    http://tampermonkey.net/
// @version      7.04
// @description  Enhanced Labouchere - Auto-detect Custom Bets (One-time use), Fixed duplicate Win detection, Math fixes
...
(function() {
    ...
    // =============================================================================
    // CONFIGURATION CONSTANTS - V7.04 Enhancement
    // =============================================================================
        ...
        generateSequence: (target, parts, uniform, integers) => {
            ...
            if (uniform) {
                ...
                for (let i = 0; i < parts - 1; i++) {
                    ...
                    sequence.push(val);
                    remaining = Utils.cleanNum(remaining - val);
                }
            ...
            let diff = Utils.cleanNum(target - sequence.reduce((a, b) => a + b, 0));
            ...
            while (Math.abs(diff) >= (integers ? 0.5 : 0.05) && iterations < maxIterations) {
                ...
                if (diff > 0) {
                    sequence[idx] = Utils.cleanNum(sequence[idx] + step);
                    diff = Utils.cleanNum(diff - step);
    ...
    class GameEngine {
        ...
        // V7.04: Enhanced with CONFIG constants and better logging, Math Fixes
        processWin() {
            ...
            this.state.totalProfit = Utils.cleanNum(this.state.totalProfit + baseProfit);
        ...
        processLoss(amount) {
            ...
            this.state.totalProfit = Utils.cleanNum(this.state.totalProfit - baseLoss);
            this.state.roundCount++;
            this.pendingBet = null;
    ...
    class OverlayUI {
        ...
        render(container) {
            ...
            div.innerHTML = `
                <div id="lt-header"><span style="display:flex;align-items:center;gap:8px;"><span style="background:#7c3aed;width:8px;height:8px;border-radius:50%"></span>LabTrack V7.04</span>
    ...
    class TornIntegration {
        constructor() {
            ...
            // V7.04 FIX: Restore lock state from SessionStorage (Persistent across reloads)
        ...
        // V7.04: Session Storage Management for Locks
        restoreLockState() {
            try {
                const data = sessionStorage.getItem('lt_lock_session');
                if (data) {
                    const session = JSON.parse(data);
                    // Expire after 10 minutes (600000ms)
                    if (Date.now() - session.timestamp < 600000) {
                         if (session.locked) {
                             this.lockResults = true;
                             this.isProcessing = false; // Reset processing flag, but keep lock
                             Logger.info('Net', 'Restored Lock State from Session');
                         }
                         if (session.lastWinner) {
                             this.lastProcessedWinner = session.lastWinner;
                             this.lastProcessedTime = session.timestamp;
                         }
                    }
                }
            } catch (e) {}
        ...
        saveLockState() {
            try {
                const session = {
                    locked: this.lockResults,
                    lastWinner: this.lastProcessedWinner,
                    timestamp: Date.now()
                };
                sessionStorage.setItem('lt_lock_session', JSON.stringify(session));
            } catch (e) {}
        }
        ...
        scanTextForResults(text) {
            ...
            try {
                ...
                // V7.04: If restored from session, we might still match this.
                if (this.lastProcessedWinner === winnerId) {
    ...
    // =============================================================================
    // INITIALIZATION - V7.04
    // =============================================================================
    Logger.info('INIT', `LabTrack Enhanced v${CONFIG.VERSION} starting...`);
    Logger.info('INIT', 'Enhancements: Math Fixes, Session Locks');
